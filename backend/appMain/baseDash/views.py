...
from .models import Contest, Prediction, Leaderboard, Score, Report, generate_slug_code
#from token_blacklist.models import OutstandingTokens
from .serializers import ContestSerializer, EntrySerializer, UserPublicSerializer, ScoreSerializer,ReportSerializer,CreateEntrySerializer,LeaderboardSerializer, ResultSerializer
...
class leaderboardList(generics.ListCreateAPIView):
    ...
    headers = {"Access-Control-Allow-Origin": "http://localhost:8000"}
    lookup_fields = ['user','entries','overall','predictions']


class leaderboardDetail(generics.RetrieveDestroyAPIView):
    authentication_classes = [authentication.TokenAuthentication]
    permission_classes = [permissions.AllowAny]
    ...
    def post(self,request,format=None ):
        if request.method == 'POST':
            try:
                ...
                print(contest.day)
                try:
                    query = Prediction.objects.get(source=user, contest=contest)
                ...
                return Response(data={'message': 'login'}, status=status.HTTP_200_OK)
...
class getActive(APIView):
    ...
    def get(self,request,format=None):
        if request.method == 'GET':
            ...
            queryset = Contest.objects.get(day=date.today(), isOpen=True)
...
class resultDetail(generics.RetrieveAPIView):
    def get(self, request, format=None):
        ...
        queryset = Prediction.objects.filter(contest=contest)
        print(queryset)
        authentication_classes = [authentication.TokenAuthentication]
        permission_classes = [permissions.AllowAny]
